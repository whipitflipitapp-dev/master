-- Atomic recipe creation RPC.
-- Postgres functions run in the caller's transaction, so any ingredient/tag/allergen
-- failure rolls back the main recipe row as well. The function is SECURITY DEFINER
-- so catalog upserts can be race-safe, but ownership is fixed from auth.uid().
