-- Demo recipes for local/staging UI (idempotent, safe to re-run).
-- Uses fixed recipe UUIDs + ON CONFLICT DO NOTHING. Inserts ingredients by name with ON CONFLICT (name) DO NOTHING.
-- Applied as the migration role (bypasses RLS); client apps should continue using the anon key — use service_role only server-side per project docs.
