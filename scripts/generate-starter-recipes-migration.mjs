...
// Flatten to exactly 100 recipes with category distribution
const TARGET = 100;
const recipes = [];
let seq = 1;
...
// Round-robin pick until 100, ensuring each category used at least once
...
// Fill to 100 favoring summer categories
...
while (recipes.length < TARGET) {
  ...
  if (queues[cat].length === 0) {
    queues[cat].push(...BY_CATEGORY[cat]);
  }
  const item = queues[cat].shift();
  recipes.push({ cat, item, seq: seq++ });
  counts[cat]++;
